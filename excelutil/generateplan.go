...
import (
	...
	"github.com/yoloz/goutils/timeutil"
)
...
func GeneratePlan(year int, months []int, filePath string) {
	...
	for _, month := range months {
		...
		if days, err = timeutil.GetMonthDay(year, month); err != nil {
			log.Fatal(err)
		}

		if startWeek, err = timeutil.GetWeekday(year, month, 1); err != nil {
			log.Fatal(err)
		}
		//month start col
		...
		for day := 1; day <= days; day++ {
			...
			//weekday: week的汉化
			weekday := string([]rune(timeutil.Weekday_zh(time.Weekday(week)))[2:])
