...
func (handler MssqlHandler) CreateTable(db *sql.DB) (sql.Result, error) {
...
//拼接insert语句
func (handler MssqlHandler) BatchInsertSql(db *sql.DB, startNum int, batchNum int) (sql.Result, error) {
	st := time.Now().Unix()
