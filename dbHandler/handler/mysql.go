...
func (handler MysqlHandler) CreateTable(db *sql.DB) (sql.Result, error) {
	...
	str := `CREATE TABLE person (sfzh varchar(100) NOT NULL,birth DATE NOT NULL,age INT NOT NULL,
	ip varchar(100) NOT NULL,post INT NOT NULL,PRIMARY KEY (sfzh))ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 
...
//拼接insert语句
func (handler MysqlHandler) BatchInsertSql(db *sql.DB, startNum int, batchNum int) (sql.Result, error) {
	st := time.Now().Unix()
