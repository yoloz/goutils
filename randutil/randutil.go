...
// MakeRandString 随机生成一个字符串（指定字符种子） 入参：（字符串长度，长度是否随机）
...
// MakeRandString2 随机生成一个字符串（任意字符） 入参：字符串长度
