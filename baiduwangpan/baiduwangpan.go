...
/*
运行报错:{"errno":2,"newno":"","request_id":4143539216373492157,"show_msg":"Param error"}
临时用
curl "https://pan.baidu.com/share/transfer?access_token=21.6207ea06baa2847b62854e80d2346b78.2592000.1578547406.590339695-17979131&shareid=1231251527&from=4094053687&sekey=MP%2FJFKWzBzE91bRmkRsukpzD0WYNBbQ3gB%2B5Bnpwdas%3D" -d 'fsidlist=[796673502398875,386941620728896]&path=%2fphoto%2f20191201三亚' -H "User-Agent: pan.baidu.com" -H "Referer:pan.baidu.com"
*/
func transferImpl(accessToken, shareid, randsk, uk, flist, targetPath string) {
	client := &http.Client{}
	var req *http.Request
	req, _ = http.NewRequest("POST",
		"https://pan.baidu.com/share/transfer?access_token="+accessToken+"&shareid="+shareid+"&from="+uk+
			"&sekey="+randsk,
		strings.NewReader("fsidlist=["+flist+"]&path="+targetPath))
	fmt.Printf("url %s -d %s", "https://pan.baidu.com/share/transfer?access_token="+accessToken+"&shareid="+shareid+"&from="+uk+
		"&sekey="+randsk, "fsidlist=["+flist+"]&path="+targetPath)
	//添加header
	req.Header.Add("User-Agent", "pan.baidu.com")
	req.Header.Add("Referer", "pan.baidu.com")
	resp, err := client.Do(req)
...
func listShareDir(accesstoken, shareid, surl, randsk, uk, path string) {
	var shareDir ShareLinkDir
	body := httpGet("https://pan.baidu.com/share/list?shareid=" + shareid + "&shorturl=" + surl + "&sekey=" +
		randsk + "&root=1")
	if err := json.Unmarshal(body, &shareDir); err != nil {
	...
	for _, sf := range shareDir.List {
		if strings.Compare(sf.Isdir, "1") == 0 {
			...
			for {
				body := httpGet("https://pan.baidu.com/share/list?shareid=" + shareid + "&shorturl=" + surl + "&sekey=" +
					randsk + "&dir=" + sf.Path + "&page=" + strconv.Itoa(page) + "&num=1000")
				...
				for i := 0; i < count; i++ {
					sfl := flist.List[i]
					if sfl.Isdir == 1 {
						listShareDir(accesstoken, shareid, surl, randsk, uk, path)
					} else {
						fids = fids + strconv.FormatInt(sfl.FsID, 10)
						if i != count-1 {
							fids = fids + ","
						}
					}
				}
				if strings.Compare(fids, "") != 0 {
					transferImpl(accesstoken, shareid, randsk, uk, fids, path)
				}
				page++
				counter += count
			}
			fmt.Printf("total transfer page %d counter %d", page, counter)
		} else {
...
//遍历文件夹下文件并转存
func transferSharelinkFiles(accessToken, targetPath string) {
	...
	listShareDir(accessToken, shareid, surl, randsk, uk, targetPath)
}
...
//技术文档:https://pan.baidu.com/union/document/entrance
func main() {
	//申请百度开发者中心应用
	...
	/* 获取外链密码
	   ...
	   验证码直接拷贝页面提示框里即可
	*/
	transferSharelinkFiles("21.6207ea06baa2847b62854e80d2346b78.2592000.1578547406.590339695-17979131",
		"%2fphoto%2f20191201三亚")
	// uk := shareLinkUK("1231251527", "yl0Ak3paNXKClQvYPBJxNg", "MP%2FJFKWzBzE91bRmkRsukpzD0WYNBbQ3gB%2B5Bnpwdas%3D")
	// transferImpl("21.6207ea06baa2847b62854e80d2346b78.2592000.1578547406.590339695-17979131",
	// 	"1231251527", "MP%2FJFKWzBzE91bRmkRsukpzD0WYNBbQ3gB%2B5Bnpwdas%3D", uk, "796673502398875,386941620728896",
	// 	"/photo/20191201三亚")
