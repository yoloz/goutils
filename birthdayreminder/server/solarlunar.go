...
//SolarToLunar 公历转农历
func SolarToLunar(solarYear, solarMonth, solarDay int) [4]int {
...
//LunarToSolar 农历转公历
//如果month是闰月,leap是true则返回的结果是闰月的第二个月对应的公历月份;leap是false则返回的结果时闰月的第一个月对应的公历月份
//month非闰月则leap不影响返回结果
func LunarToSolar(year, month, day int, leap bool) [3]int {
	var sum int = 0
	for i := 1900; i < year; i++ {
		sum += yearSum[i]
	}
...
func getTimestamp(timeStr string) (int64, error) {
	loc, err := time.LoadLocation("Asia/Shanghai") //time.LoadLocation("Local")
	if err != nil {
		return -1, fmt.Errorf("time.LoadLocation %s", err)
	}
	Time, err := time.ParseInLocation("2006-01-02", timeStr, loc)
	if err != nil {
		return -1, fmt.Errorf("time.ParseInLocation %s", err)
	}
...
//GetDayGanZhiIndex 获取流日天干地支的索引值,从0开始
